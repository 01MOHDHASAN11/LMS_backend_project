...
export const signin = async (req, res) => {
  try {
    ...
    const accessToken = jwt.sign({ userId: user._id }, process.env.ACCESS_TOKEN, { expiresIn: "15m" });
    ...
    await redisClient.set(`refreshToken:${user._id}`, refreshToken, { EX: 7 * 24 * 3600 }); // 7 days
    await redisClient.set(`session:${user._id}`,`${user.email}`,{EX:7*24*60*60})
    // Set cookie
...
export const refreshToken = async (req, res) => {
  try {
    ...
    const newAccessToken = jwt.sign({ userId: decoded.userId }, process.env.ACCESS_TOKEN, { expiresIn: "15m" });

    // Optional: generate new refreshToken (rotate refresh tokens)
    const newRefreshToken = jwt.sign({ userId: decoded.userId }, process.env.REFRESH_TOKEN, { expiresIn: "7d" });

    // Store in Redis
    await redisClient.set(`accessToken:${decoded.userId}`, newAccessToken, { EX: 900 });
    await redisClient.set(`refreshToken:${decoded.userId}`, newRefreshToken, { EX: 7 * 24 * 3600 });
