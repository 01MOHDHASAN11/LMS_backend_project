...
export const unBlockUserRequest = async (req, res) => {
  try {
    ...
    const existingUserRequest = await unblockRequestModel.findOne({
      user: user._id,
      status: "pending",
    });
    if (existingUserRequest) {
      return res
        .status(400)
        .json({ message: "Unblock request already pending" });
    }
    ...
    res
      ...
      .json({
        message:
          "We received your unblock request. We will get back to you through mail",
      });
  } catch (error) {
