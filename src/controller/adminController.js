...
import {
  blockUser,
  updateUserBlockStatusValidation,
} from "../validation/blockUser.validation.js";
...
export const blockUserController = async (req, res) => {
  try {
    ...
    if (blockType === "permanent") {
      ...
      return res
        .status(200)
        .json({ message: "User blocked successfully", userData: user });
    }
...
export const updateUserBlockStatus = async (req, res) => {
  try {
    ...
    if (error)
      return res.status(400).json({ message: error.details[0].message });
    const blockedUser = await unblockRequestModel
      .findById(requestId)
    ...
    if (status === "approved") {
      (blockedUser.status = "approved"),
        (blockedUser.message =
          adminMessage || "User account has been approved"),
        (blockedUser.reviewedAt = Date.now());
      await blockedUser.save();
      (user.isBlocked = false), (user.blockUntil = null), await user.save();
    }
    if (status === "rejected") {
    ...
    await emailQueue.add(
      "admin-response-on-unblock-request",
      {
    ...
    if (status === "rejected")
      return res.status(200).json({ message: "User unblock request rejected" });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Internal server error", error });
  }
...
export const getPendingUserRequest = async (req, res) => {
  try {
    ...
    const skip = (page - 1) * limit;
    const pendingUser = await unblockRequestModel
      .find({ status: "pending" })
      .limit(limit)
      .skip(skip);
    if (!pendingUser)
...
export const getSingleVerificationRequest = async (req, res) => {
  try {
    const { id } = req.params;
    const requestData = await verificationRequestModel
      .findById(id)
      .populate("user", "name resumeUrl email");
    if (!requestData)
...
export const updateInstructorVerificationRequest = async (req, res) => {
  try {
    const { id } = req.params;
    const { status, adminMessage = "" } = req.body;
    const { error } = updateInstructorVerificationRequestValidation.validate(
      req.body
    );
    if (error)
      return res.status(400).json({ message: error.details[0].message });
    const updateRequest = await verificationRequestModel
      .findByIdAndUpdate(
        id,
        { status, adminMessage, reviewedAt: Date.now() },
        { new: true }
      )
      .populate("user", "name email");
    if (!updateRequest)
    ...
    const instructor = await userAuth.findByIdAndUpdate(
      updateRequest.user,
      { instructorVerified: instructorVerifiedBoolean },
      { new: true }
    );
    if (!instructor)
      return res.status(404).json({ message: "Instructor not found" });
    await emailQueue.add(
      "instructor-verification-response",
      {
...
export const reviewCourseRequest = async (req, res) => {
  ...
  try {
    ...
    const request = await courseReviewRequestModel
      .findById({
        _id: requestId,
      })
      .session(session);
    ...
    if (!existingCourse || existingCourse.status !== "review") {
      ...
      return res
        ...
        .json({
          success: false,
          message: "Invalid course id or course not in review state",
        });
    }
    if (action === "approved") {
    ...
    res
      .status(200)
      .json({ success: true, message: "Request review successful" });
  } catch (error) {
    console.error(error);
    await session.abortTransaction();
    return res
